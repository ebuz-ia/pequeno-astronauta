...
Game.scenes.PLANET_EXPLORE = {
  ...
  update: function(dt) {
    ...
    if (this.alienSpawnTimer <= 0 && this.planetIndex > 0) {
      ...
      var foodTypes = [null, 'fungo_lunar', 'carne_marciana', 'fruta_acida', 'gelo_nutritivo'];
      var foodHungers = [0, 15, 25, 18, 22];
      ...
      Game.EntityManager.add('enemies', {
        x: alienX, y: alienGroundY - 18, radius: 15, hp: 20 + planetIdx * 5,
        maxHp: 20 + planetIdx * 5,
        ...
        update: function(dt2) {
          ...
          if (Math.abs(dx) > 30) this.x += this.facing * (40 + planetIdx * 8) * dt2;
          var gx = Math.min(Math.floor(this.x), Game.scenes.PLANET_EXPLORE.terrain.length - 1);
          if (gx >= 0 && gx < Game.scenes.PLANET_EXPLORE.terrain.length) this.y = Game.scenes.PLANET_EXPLORE.terrain[gx] - 18;
          if (Math.abs(dx) < 40 && Math.abs(ast.y - this.y) < 45) {
            if (!this._hitCooldown || this._hitCooldown <= 0) {
              ast.hp = Math.max(0, ast.hp - 10); this._hitCooldown = 0.8;
              Game.triggerShake(4, 0.15);
              if (Game.Audio) Game.Audio.sfx.damage();
        ...
        takeDamage: function(dmg) {
          ...
          if (this.hp <= 0) {
            ...
            // Drop comida (40% chance)
            if (Math.random() < 0.4 && Game.scenes.PLANET_EXPLORE.foodItems) {
              var ft = foodTypes[pi] || 'carne';
              var fh = foodHungers[pi] || 20;
              Game.scenes.PLANET_EXPLORE.foodItems.push({
                x: this.x, y: this.y, type: ft, hunger: fh, collected: false
              });
            }
            // Drop municao (30% chance)
            if (Math.random() < 0.3) {
